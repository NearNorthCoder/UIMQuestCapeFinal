...
public class BuyFireRunesState extends AbstractState {
    ...
    private static final int FIRE_RUNES_TO_BUY = 100;

    public BuyFireRunesState(UIMQuestCape script) {
        super(script, "BuyFireRunesState");
    }

    @Override
    public int execute() {
        // Check if we already have fire runes
        if (Inventory.contains("Fire rune")) {
            Logger.log("Already have fire runes");
            complete();
            return 600;
        }

        // Check if we have enough money
        if (Inventory.count("Coins") < 400) {
            Logger.log("Not enough coins to buy fire runes");
            return 600;
        }
    ...
    public boolean canExecute() {
        return !Inventory.contains("Fire rune") && Inventory.count("Coins") >= 400;
    }
}