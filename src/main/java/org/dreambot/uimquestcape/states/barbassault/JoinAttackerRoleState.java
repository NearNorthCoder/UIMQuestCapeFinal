...
public class JoinAttackerRoleState extends AbstractState {
    ...
    @Override
    public int execute() {
        // Check if we've already selected the attacker role
        if (roleSelected) {
            Logger.log("Attacker role already selected");
            complete();
            return 600;
        }
        ...
        // Find and interact with Attacker role NPC
        NPC attackerCaptain = NPCs.closest(npc -> 
            npc != null && 
            npc.getName() != null && 
            npc.getName().contains("Captain") && 
            npc.hasAction("Talk-to") &&
            npc.getOverheadText() != null &&
            npc.getOverheadText().contains("Attacker")
        );
        
        if (attackerCaptain != null) {
            Logger.log("Talking to Attacker captain");
            attackerCaptain.interact("Talk-to");
            Sleep.sleepUntil(Dialogues::inDialogue, 5000);
            return 600;
        } else {
            Logger.log("Attacker captain not found, checking recruitment scroll board");
    ...
    private int handleDialogue() {
        ...
        if (options != null) {
            // Look for options related to joining the attacker role
            for (int i = 0; i < options.length; i++) {
                if (options[i].contains("Join") || 
                    options[i].contains("I'll be an attacker") || 
